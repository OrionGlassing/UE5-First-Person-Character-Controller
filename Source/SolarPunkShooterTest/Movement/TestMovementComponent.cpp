	...
	FActorComponentTickFunction* ThisTickFunction)
{
	...
	InputVector = BaseCharacterOwner->GetLastMovementInputVector(); // maybe change to last consumed inputvctor
	...
	if (IsWalking())
	{
		...
		{
			if (Velocity.Size() > Tac_MaxSpeed)
			{
				EnterTacSprint();
			}
			else if (Velocity.Size() > Sprint_MaxSpeed)
			{
				Sprint();
			}
		}
...
void UTestMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	...
	if (bTransitionFinished_MY)
	{
		...
		{
			// to make short mantle smoother we can remove the transitionRMS
			// and just go straight to the mantleRMS (need a complex work around tho)
...
void UTestMovementComponent::ForceResetMovement()
{
	...
	MaxWalkSpeed = Walk_MaxSpeed;
	BaseCharacterOwner->IsTacSprinting = false;
	TacSprintDeplete = false;
	BaseCharacterOwner->IsSprinting = false;
	BaseCharacterOwner->ForwardMulti = DefaultForwardMulti;
	BaseCharacterOwner->StrafeMulti = DefaultStrafeMulti;
	BaseCharacterOwner->BackMulti = DefaultBackMulti;
}
...
bool UTestMovementComponent::Sprint()
{
	...
	CanResetSprint = true;
	BaseCharacterOwner->IsSprinting = true;
	BaseCharacterOwner->ForwardMulti = DefaultForwardMulti;
...
void UTestMovementComponent::EnterTacSprint()
{
	if (IsWalking() && !TacSprintOnCooldown) {
		...
		BaseCharacterOwner->IsTacSprinting = true;
...
void UTestMovementComponent::ExitTacSprint()
{
	...
	if (BaseCharacterOwner->IsTacSprinting)
	{
...
void UTestMovementComponent::ResetTacSprint()
{
	...
	/* Sliding NOTES!!
 *	FOUND A WEIRD WAY TO GET STUCK WITH SLIDING CONTROL VALUES!
 *		- I think i found it!
 *		- I added another section for when in the air, in this section there are no exit commands...
 *		- so if we are in the air when we exit slide, we will stop sliding without exiting
 *		- maybe this isn't it tho bc how would we exit in the first place...??!
 *		- OR
 *		- The capsule is being forced to reset for a caspule related reason
 *		- but we are technically still sliding...
 *		- so we should make it so if the capsule resets, and we are still sliding, we call exit slide)
 *		- but this also makes no sense (we would still exit bc we are going too slow)
 * also can possibly still get stuck with slide control values after exit (possibly fixed)
*/
void UTestMovementComponent::EnterSlide()
{
	...
	HalfCapsuleSize();
	BaseCharacterOwner->IsSliding = true;
...
void UTestMovementComponent::TickSlide(float DeltaTime)
{
	...
	if (BaseCharacterOwner->IsSliding)
	{
		...
		if (IsMovingOnGround())
		{
			...
			if ((Velocity.Size2D() < Slide_MinVelocity) || (BaseCharacterOwner->IsSprinting && Velocity.Size2D() < Sprint_MaxSpeed))
			{
...
void UTestMovementComponent::ExitSlide()
{
	...
	*/
	...
	ResetCapsuleSize();
	BaseCharacterOwner->IsSliding = false;
...
/* Wall Running Notes:
 *	IT IS WORKING LEGENDARILY!!! - nearly...nearly still...NOT ANY MORE!!!
 *
 *	IT'S ALL FIXED...ALL OF IT...EVERYTHING!!!!!
 *	all we had to do was handle collisions
 *	
 *	another note:
 *	entering can be a bit tricky, consistent, but tricky...
 *	this can either be good to make it more precise and techy, or bad if it wants to be more smooth everytime...
*/
...
/* Mantle Notes:
 *
 *	This would work great, if it wasn't for the fact that we don't actually have a root bone to do any root motion!
 *	However, I think this approach will still work great!
 *	Fixes 1:
 *		- Remove all mentions of root motion / animation
 *		- Find a way to lerp the position of the capsule to the spot we want it at
 *		- Ideally it would still have collision, where we don;t just go through the wall,
 *			but this would only really be an issue if the camera goes through the wall
 *	Fixes 2:
 *		- Find a way to make a root motion source
 *			- Make a skeleton asset for the character and make an animation.
 *			OR
 *			- Find a way to directly animate the capsule.
 *
 *	CURRENT FIX ATTEMPT:
 *		- Make 2 ForceTo RootMotions
 *		- The first one "Transition" gets us into the correct position to do the full mantle
 *			- Set a constant position relative to the top of the wall
 *			- Position needs to be high enough so that out head never clips into the wall
 *				ideally, it would have half of the capsule half height above the wall
 *			- Move there relative to how fast the character is moving
 *				Velocity  = distance / time
 *				We have our current velocity, and we can get the distance between character and target pos, so find time.
 *		- The second will be the mantle to the top of the wall
 *			- Scale the speed of this based on our Z Velocity
 *				- if we are going fast up into it, go faster
 *				- if we are going fast down into it, go slower
 *			
 *			
 
    Note: Check if the capsule is currently being warped into the start animation position
			if it is, then there's a way to move the capsule, and all we gotta do is use that.
...
bool UTestMovementComponent::TryMantle()
{
	...
	// supposedly the location of the top of the wall, where we want the capsule to go
	// it's either this or just SurfaceHit.Location
...
void UTestMovementComponent::EndJump()
{
	...
	if (BaseCharacterOwner->IsSprinting)
	{
		BaseCharacterOwner->StrafeMulti = Sprint_StrafeMulti;
		BaseCharacterOwner->BackMulti = Sprint_BackMulti;
	}
...
/* Notes:
 * Both Functions are using the pre-built functionality for CROUCHING
 * This means they are using/doing several things that are not necessary
 * such as creating a default character to get values, rather than just setting float variables and using those
 * It also uses the Crouch helper functions OnCrouchStart/End from the character class
 * Overall this means that the functions work, but are not optimized OR completely understood
 * What I mean is, it's possible somewhere down the line things are being done that I dont want, like Crouching
 * This is all worth re-writing including the character crouch helpers!
*/
